...
class Bank {
    ...
    public static Bank loadData() {
        Bank bank = null;
        try (BufferedReader reader = new BufferedReader(new FileReader("bank_data.txt"))) {
            String line;
            Branch currentBranch = null;

            while ((line = reader.readLine()) != null) {
                if (line.startsWith("Bank: ")) {
                    bank = new Bank(line.substring(6));
                } else if (line.startsWith("Branch: ")) {
                    currentBranch = new Branch(line.substring(8), bank);
                    bank.addBranch(currentBranch);
                } else if (line.startsWith("Deposit: ")) {
                    String[] parts = line.substring(9).split(", ");
                    if (parts.length == 2 && currentBranch != null) {
                        currentBranch.addDeposit(new Deposit(parts[0], currentBranch, Double.parseDouble(parts[1])));
                    }
                }
            }
        } catch (IOException e) {
        ...
        return bank != null ? bank : new Bank("Global Bank");
    }
